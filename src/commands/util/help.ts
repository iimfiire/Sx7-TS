...
export default new Command({
    ...
    execute: ({message, args, client, prefix}) => {
        ...
        if(!args[0]) {
            ...
            helpEmbed.setFooter(`Run ${prefix}help [command] for more information on a specific command.`)
            ...
            categories.forEach((category) => {
                ...
                helpEmbed.addField(category, categoryCommands.map(cmd => cmd.name).join(", "))
            })
        ...
        if(args[0]) {
            if(!client.commands.get(args[0]) || client.commands.get(client.aliases.get(args[0]))) {
                return message.reply({embeds: [client.error({message, data: `Command not found. Make sure you spelled it correctly or that it exists.`})]})
            }
            else {
                ...
                helpEmbed.addFields([
                    ...
                    {name: 'Aliases', value: command.aliases && command.aliases.length > 0 ? "• " + command.aliases.join("\n• ") : 'None', inline: true},
